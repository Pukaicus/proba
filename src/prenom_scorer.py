...
def charger_prenoms_depuis_csv(csv_path):
    ...
    # Filtrer prénoms de moins de 3 lettres (souvent des faux positifs)
...
def get_prenom_score(prenom, rarity_dict, bins, email=None, nom=None, texte=None):
    ...
    # Filtre prénom trop court : ne donne aucun score ni bonus
    if len(p_clean) < 3:
        return {
            "prenom": prenom,
            "base_score": 0,
    ...
    if rare_val is None:
        ...
        base_score = score_rarete(rare_val, bins)
    ...
    if texte:
        ...
        # Bonus +2 si prénom présent dans un contexte réseaux sociaux
